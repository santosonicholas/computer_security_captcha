...
if(isset($_POST['form_submit'])) {
    ...
    $roles = mysqli_real_escape_string($connString, trim($_POST['roles']));

    //Error trapping for similar username and Email
    $sql_username = "SELECT username FROM staff WHERE username = '$username'";
    $sql_email = "SELECT email FROM staff WHERE email = '$email'";
    ...
    if($flag ==0){
        ...
        $sql = "INSERT INTO `staff` (`first_name`,`last_name`,`username`, `password`, `gender`, `email`, `roles`) VALUES
                    ('".$first_name."','".$last_name."','".$username."', '".$encrypted_password."', '".$gender."', '".$email."','".$roles."');";
        ...
        header("Location: add_account_staff.php");
    }

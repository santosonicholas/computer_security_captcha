...
function simple_php_captcha($config = array()) {
    ...
    if( empty($captcha_config['code']) ) {
        ...
        while( strlen($captcha_config['code']) < $length ) { //random captcha code by using substring, cut the alphabet into 1 until length are full
            $captcha_config['code'] .= substr($captcha_config['characters'], mt_rand() % (strlen($captcha_config['characters'])), 1);
        }
        $_SESSION['code'] = $captcha_config['code'];
    }

    // Generate HTML for image src
    if ( strpos($_SERVER['SCRIPT_FILENAME'], $_SERVER['DOCUMENT_ROOT']) ) {
        $image_src = substr(__FILE__, strlen( realpath($_SERVER['DOCUMENT_ROOT']) )) . '?_CAPTCHA&amp;t=' . urlencode(microtime()); ///captcha_dave/simple-php-captcha.php?_CAPTCHA&t=0.40416700+1581835707
        $image_src = '/' . ltrim(preg_replace('/\\\\/', '/', $image_src), '/'); // mengganti "\" ke "/"
    } else {
